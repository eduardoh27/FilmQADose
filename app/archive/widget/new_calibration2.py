...
class CalibrationWidget(QWidget):
    """
    Interactive widget to select ROIs on a TIFF film, assign dose,
    and run calibration with embedded curve display.
    """
    def __init__(self, tif_path, calib_type, parent=None):
        ...
        # View container for image and later curve
        ...
        self.view_layout.addWidget(self.gv)
        self._setup_viewbox()
    ...
    def ask_dose(self, default_value=100):
        dose, ok = QtWidgets.QInputDialog.getInt(
            self, "Enter dose", "Associated dose (cGy):", value=default_value, minValue=0
        )
        return ok, int(dose)
    ...
    def refresh_roi_data(self):
        ...
        for roi in self.rois:
            ...
            self.roi_data.append((int(p.x()), int(p.y()), int(s.x()), int(s.y()), roi.dose))
    ...
    def on_calibrate(self):
        # Instantiate calibration logic
        calib = FilmCalibration(self.tif_path, fitting_function_name=self.calib_type)
        for x, y, w, h, dose in getattr(self, 'roi_data', []):
            calib.add_roi(dose, x, y, w, h)
        print(calib.get_rois_by_dose())
        calib.calibrate()

        # Graph and embed
        fig = calib.graph_calibration_curve()
        # Save JSON
        date_tag = datetime.now().strftime("%Y%m%d_%H%M%S")
        json_path = os.path.join(os.path.dirname(self.tif_path), f"calibration_{date_tag}.json")
        calib.to_json(json_path)

        # Inform user
        QMessageBox.information(self, "Calibration Saved",
            f"Calibration saved to {json_path}")

        # Replace view with the calibration curve
        # Clear previous view
