...
class CalibrationDialog(QDialog):
    """
    Dialog to select a single TIFF film and calibration type.
    """
...
class CalibrationWidget(QWidget):
    """
    Widget to draw ROIs on TIFF and calibrate with an embedded Matplotlib plot.
    """
    def __init__(self, tif_path, calib_type, parent=None):
        ...
        # View container (for graph/image)
        self.view_container = QVBoxLayout()
        self.gv = pg.GraphicsView()
        self.vb = pg.ViewBox()
        self.gv.setCentralItem(self.vb)
        self.view_container.addWidget(self.gv)
        self.main_layout.addLayout(self.view_container, stretch=4)
        ...
        self.ctrl_layout.addWidget(self.calibrate_btn)
        self.ctrl_layout.addStretch()
        self.main_layout.addLayout(self.ctrl_layout, stretch=1)
    ...
    def refresh_roi_data(self):
        ...
        for roi in self.rois:
            ...
            self.roi_data.append((int(p.x()), int(p.y()),
    ...
    def on_calibrate(self):
        # Hide calibrate button immediately
        self.calibrate_btn.hide()
        # Remove controls layout to free space
        self.main_layout.removeItem(self.ctrl_layout)
        ...
        # Show wait message
        self.wait_lbl = QLabel("Calibrating, please wait...", self)
        self.wait_lbl.setAlignment(QtCore.Qt.AlignCenter)
        self.view_container.addWidget(self.wait_lbl)
        QtWidgets.QApplication.processEvents()

        # Calibration logic
        calibration = FilmCalibration(self.tif_path, fitting_function_name=self.calib_type)
        for x, y, w, h, dose in self.roi_data:
            calibration.add_roi(dose/100, x, y, w, h)
        calibration.calibrate()

        # Save JSON
        date_str = datetime.now().strftime("%Y%m%d")
        json_path = f"calibracion_{date_str}.json"
        calibration.to_json(json_path)
        QMessageBox.information(self, "Calibration Saved", f"Calibration saved to {json_path}")

        # Replace wait label with plot
        fig = calibration.graph_calibration_curve()
        canvas = FigureCanvas(fig)
        self.wait_lbl.setParent(None)
        self.view_container.addWidget(canvas)

        # 1) Guardar la ruta absoluta en la metadata del widget
        self.metadata["json_path"] = os.path.abspath(json_path)
        # 2) Forzar refresco del panel izquierdo
        main_win = self.window()
        main_win.update_left_panel(main_win.tab_widget.currentIndex())
